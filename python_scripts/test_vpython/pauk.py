        ...
        if len(animation) == 0:
            ...
            for i in range(1, len(animation)):
                if animation[i][0] >= time:
                    return vector(animation[i-1][1]) + (vector(animation[i][1]) - vector(animation[i-1][1])) * (time - animation[i-1][0]) / (animation[i][0] - animation[i-1][0])
            ...
            if self.first_step:
                ...
                if self.move_angle == 0:
                    ...
                    if 0:
                        ...
                        self.body_pos_anim = [(0.0, vector(self.get_pos())), (self.move_time, self.get_pos() + self.move_direction)]
                    ...
                    for id in [0,1,2,3]:
                        leg = self.legs[id]
                        new_pos_1 = self.get_pos() + rotate(vector=(leg.get_pos() - self.get_pos()), angle=self.move_angle / 2, axis=self.f_1.up)
                        new_pos_2 = self.get_pos() + rotate(vector=(leg.get_pos() - self.get_pos()), angle=self.move_angle, axis=self.f_1.up)
                        leg.anim = [(0.0 + dt, vector(leg.get_pos())),
...
if __name__ == '__main__':
    ...
    def mousedown(event):
        if  event.shift == False:
            ...
            if selected_obj is not None:
                ...
                if leg is not None:
                    offset = leg.get_pos() - scene.mouse.pos
                elif pauk.is_body(selected_obj):
                    offset = pauk.get_pos() - scene.mouse.pos
    ...
    def mousemove(event):
        ...
        if selected_obj is not None:
            ...
            if leg is not None:
                leg.set_pos(scene.mouse.pos + offset)
            elif pauk.is_body(selected_obj):
                legs_pos = [l.get_pos() for l in pauk.legs]
                pauk.set_pos(scene.mouse.pos + offset)
